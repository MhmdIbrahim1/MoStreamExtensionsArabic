...
class FaselHD : MainAPI() {
    ...
    override var mainUrl = "https://faselhd.center"
    ...
    override val mainPage = mainPageOf(
        ...
        "$mainUrl/movies_top_views/page/" to "Top Movies",
        "$mainUrl/series/page/" to "Series",
        "$mainUrl/recent_series/page/" to "Recently Added",
        "$mainUrl/asian-movies/page/0" to "Asian Movies",
        "$mainUrl/anime/page/" to "Anime",
        "$mainUrl/dubbed-movies/page/" to "Dubbed Arabic Movies",
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val duration =
            doc.select("div[id=\"singleList\"] div[class=\"col-xl-2 col-lg-2 col-md-3 col-sm-3\"]").firstOrNull {
                it.text().contains("مدة|توقيت".toRegex())
            }?.text()?.getIntFromText()

        val tags =
            doc.select("div[id=\"singleList\"] div[class=\"col-xl-2 col-lg-2 col-md-3 col-sm-3\"]:contains(تصنيف الفيلم) a")
                .map {
                    it.text()
                }
        ...
        return if (isMovie) {
            ...
            val seasons = doc.select("div[id=\"seasonList\"] div[class=\"col-xl-2 col-lg-3 col-md-6\"] div.seasonDiv")
            seasons.forEach { season ->
                val seasonUrl = "$mainUrl/?p=${season.attr("onclick").substringAfterLast("=").replace("'", "")}"
                var seasonDocument = app.get(seasonUrl).document

                if (seasonDocument.select("title").text() == "Just a moment...") {
                    seasonDocument =
                        app.get("$alternativeUrl/?p=${season.attr("data-href")}", interceptor = cfKiller).document
                }

                seasonDocument.select("div.epAll a").map { episodeLink ->
                    episodes.add(
                        Episode(
                            episodeLink.attr("href"),
                            episodeLink.text(),
                            season.select("div.title").text().getIntFromText(),
                            episodeLink.text().getIntFromText()
                        )
                    )
                }
            }
